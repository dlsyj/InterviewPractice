...
public class lc_133_cloneGraph {
    ...
    public UndirectedGraphNode cloneGraph(UndirectedGraphNode node) {
        ...
        for (UndirectedGraphNode each : node.neighbors){
            copy.neighbors.add(cloneGraph(each));   // recursively
        }
        return copy;
    }
    ...
    // BFS
    private Map<UndirectedGraphNode, UndirectedGraphNode> map2 = new HashMap<>();
        ...
        while (!queue.isEmpty()){
            ...
            for(UndirectedGraphNode neigh : curr.neighbors){
                if (map.containsKey(neigh)){
                    ...
                    map.get(curr).neighbors.add(neighCopy);
                    queue.add(neigh);
                }
            }
        }
